...
algebra.multiplicativeInverse = function (x) {
  if (utils.empty(x)) { return Interval.empty(); }
  if (utils.zeroIn(x)) {
    if (x.lo !== 0) {
      if (x.hi !== 0) {
 ...
 * Computes x^power
 * @param {Interval} x
 * @param {number} power An integer power
 * @returns {Interval}
 */
algebra.pow = function (x, power) {
  ...
  if (x.hi < 0) {
    // [negative, negative]
    // assume that both operations will yield a positive number
    // since the power is even, if not then just switch the sign and order
